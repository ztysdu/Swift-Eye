...
import mmcv
from mmrotate.core import eval_rbbox_map, obb2poly_np, poly2obb_np,obb2poly,poly2obb
...
class gazeSequenceDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.sequence_collate_fn =partial(collate_sequence, samples_per_gpu= 1)
        if not test_mode:
    ...
    def load_sequence_annotations(self, df):
        ...
        for i in range(len(df)):
            # pair_data_infos.append([self.load_image_annotation(df['search_path'].iloc[i],df['occlusion_poly'].iloc[i])])#load template and search image
            pair_data_infos.append([self.load_image_annotation(df['image_path'].iloc[i],df['poly'].iloc[i])])#load template and search image
    ...
    def load_image_annotation(self, image_path,poly):
        ...
        data_info['ann'] = {}
        poly_tensor=torch.tensor(poly).reshape(-1,8)
        obb=poly2obb(poly_tensor,"le90").reshape(-1).numpy().tolist()
        x, y, w, h, a = obb
        gt_bboxes = []
        gt_polygons = []          
